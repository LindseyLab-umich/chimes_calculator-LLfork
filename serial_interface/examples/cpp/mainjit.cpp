...
Compile with: 
    ...
    g++ -O3 -std=c++11 -o example main.cpp \serial_jit_interface.cpp \
    chimesFF.cpp
 Run with: 
    ./example <xyz file> <allow replictes (0/1 or true/false)>
...
int main(int argc, char **argv)
{
	...
	if(argc == 3)
		if((strncmp(argv[2],"true",4) == 0) || (strncmp(argv[2],"True",4) == 0) || (strncmp(argv[2],"TRUE",4) == 0) || (strncmp(argv[2],"1"   ,1) == 0))
			is_small = true;
    ...
    serial_jit_interface chimes(is_small);        // Create an instance of the serial interface
    
    chimes.init_chimesJIT(0);    // Initialize
    ...
    cout << "Success! " << endl;
    cout<< setw(21) ;
    cout << "Energy (kcal/mol):    " << endl << "\t" << scientific << setprecision(14) << energy << endl;

    cout << "Stress tensors (GPa): " << endl;
    cout << "\ts_xx: " << scientific << setprecision(14) << stress[0]*6.9479 << endl;
    cout << "\ts_yy: " << scientific << setprecision(14) << stress[4]*6.9479 << endl;
    cout << "\ts_zz: " << scientific << setprecision(14) << stress[8]*6.9479 << endl;
    cout << "\ts_xy: " << scientific << setprecision(14) << stress[1]*6.9479 << endl;
    cout << "\ts_xz: " << scientific << setprecision(14) << stress[2]*6.9479 << endl;
    cout << "\ts_yz: " << scientific << setprecision(14) << stress[5]*6.9479 << endl;
    
    cout << "Forces (kcal/mol/A): " << endl;
    for(int i=0; i<natoms; i++)
        cout << "\t"  << scientific << setprecision(14) << force[i][0] << "\t" << force[i][1] << "\t" << force[i][2] << endl;
