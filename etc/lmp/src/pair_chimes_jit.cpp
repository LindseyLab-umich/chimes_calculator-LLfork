...
PairCHIMES::PairCHIMES(LAMMPS *lmp) : Pair(lmp)
{
	...
	// 2, 3, and 4-body vars for chimesJIT access
	...
	{ 
		std::cout << std::endl;
		std::cout << "************************* WARNING (pair_style chimesJIT) ************************" << std::endl;
...
void PairCHIMES::coeff(int narg, char **arg)
{
  ...
  if (chimes_calculator.rank == 0) {
    if ( narg == 3 ) {
      std::cout << "Warning: using ChimesJIT - input file " << arg[2] << " is ignored\n" ;
    }
  }
	
  // chimesJIT_paramfile = arg[2]; 
	
  chimes_calculator.read_parameters() ;
...
{
	// Vars for access to chimesJIT compute_XB functions
	...
	int 	*type  = atom -> type;		// Acces to system atom types (countng starts from 1, chimesJIT class expects counting from 0!)
	...
	{
		if (chimes_calculator.rank == 0)
			std::cout << "Updating chimesJIT neighbor lists..." << std::endl;
	...
	for (ii = 0; ii < inum; ii++)		// Loop over the atoms owned by the current process
	{
		...
		for (jj = 0; jj < jnum; jj++)			// Loop over neighbors of i
		{
			...
			typ_idxs_2b[0] = chimes_type[type[i]-1];		// Type (index) of the current atom... subtract 1 to account for chimesJIT vs LAMMPS numbering convention
