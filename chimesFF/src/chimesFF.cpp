			  ...
			  vector<double> & stress, double & energy, chimes2BTmp &tmp)
{
	...
	set_cheby_polys(Tn, Tnd, dx,
					pair_idx, chimes_2b_cutoff[pair_idx][0],
			  ...
			  chimes3BTmp &tmp)
{
	...
	poly_3B(&poly, dpoly_dx,
			ncoeffs_3b[tripidx], chimes_3b_params[tripidx],
			mapped_pair_idx, chimes_3b_powers[tripidx],
			Tn_ij, Tn_ik, Tn_jk,
			  ...
			  vector<double> & force, vector<double> & stress, double & energy, chimes4BTmp &tmp)
{
	...
	poly_4B(&poly, dpoly_dx,
			ncoeffs_4b[quadidx],
			chimes_4b_params[quadidx],
			mapped_pair_idx,
			chimes_4b_powers[quadidx],
			Tn_ij,	Tn_ik, Tn_il,
			Tn_jk,	Tn_jl, Tn_kl,
			Tnd_ij, Tnd_ik, Tnd_il,
			Tnd_jk, Tnd_jl, Tnd_kl
		   ) ;
...
void chimesFF::poly_2B(double *e, double *f0, int ncoeffs_2b, vector<double> &chimes_2b_params, vector<int> &chimes_2b_pows,
					   vector<double> &Tn, vector<double> &Tnd)
...
void chimesFF::poly_3B(double *e, double *f,
			   int ncoeffs_3b,
			   vector<double> & chimes_3b_params,
...
void chimesFF::poly_4B(double *e, double *f,
			   int ncoeffs_4b,
			   vector<double> & chimes_4b_params,
...
// (LEF) 3/11/26
{
	...
	*e = 0 ;
	...
	for(int coeffs=0; coeffs<ncoeffs_4b; coeffs++)
	{
		...
		for (int i=0; i<npairs; i++)
			powers[i] = chimes_4b_powers[coeffs][mapped_pair_idx[i]];
