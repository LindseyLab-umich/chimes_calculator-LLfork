...
// Notes:
//
// 1. A Morse-style coordinate transformation is hard-coded (see set_cheby_vars) 
...
private:
  ...
  //inline bool set_cheby_var(double dx, const int pair_idx,
  //const double inner_cutoff, const double outer_cutoff, const int bodiedness_idx,
  //double &chvar, double &dchvar_dx) ;

  inline bool set_cheby_var(double dx, const int pair_idx,
                            const double inner_cutoff, const double outer_cutoff,
                            const double x_min, const double x_max, const int bodiedness_idx,
                            double &chvar, double &dchvar_dx) ;
  ...
  void set_polys_out_of_range(vector<double> &Tn, vector<double> &Tnd, double dx, double x,
    ...
    void calc_3B_xmin_xmax() ;
    void calc_4B_xmin_xmax() ;
  
};



inline void chimesJIT::get_fcut(const double dx, const double outer_cutoff, double & fcut, double & fcutderiv)
{
...
inline void chimesJIT::get_penalty(const double dx, const int & pair_idx, double & E_penalty, double & force_scalar)
{
    ...
    {        
        ...
        cout << "chimesJIT: " << "Warning: Adding penalty in 2B Cheby calc, r < rmin+penalty_dist " << fixed 
             << dx << " " 
             << chimes_2b_cutoff[pair_idx][0] + penalty_params[0]  
             << " pair type: " << pair_idx << endl;
        cout << "chimesJIT: " << "\t...Penalty potential = "<< E_penalty << endl;
    }   
}

inline void chimesJIT::build_atom_and_pair_mappers(const int natoms, const int npairs, const vector<int> & typ_idxs,
...
inline bool chimesJIT::set_cheby_var(double dx, const int pair_idx,
                            const double inner_cutoff, const double outer_cutoff,
                            const double x_min, const double x_max, const int bodiedness_idx,
                            double &chvar, double &dchvar_dx)
{
  // Currently assumes a Morse-style transformation has been requested
  // In this version the xmin and xmax are stored in the cutoff vector for efficiency.
  
  // Sets the value of the Chebyshev variable chvar and its derivative
  // with respect to the interatomic distance  (dchvar_dx)
  // Returns true if the interaction is within range.
  ...
  chvar  = (exprlen - x_avg)/x_diff;
  dchvar_dx = (-exprlen/morse_var[pair_idx])/x_diff;   
